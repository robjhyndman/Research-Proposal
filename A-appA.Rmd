...
$$
  ...
  \end{bmatrix}
$$
Transform back to $\bm{v}_i$ using: $\bm{v}_i = \exp(\bm{w}_i) \cdot v_{iD}$
...
$$
  \text{clr}(\bm{v}_i) = \Big(\text{ln}(\frac{v_{i1}}{g(\bm{v}_i)}),\dots,\text{ln}(\frac{v_{iD}}{g(\bm{v}_i)}))\Big) \\ = \frac{\text{ln}(\bm{x}_i)}{D} \cdot
   ...
   \end{bmatrix}
$$
Transform back to $\bm{v}_i$ using: $v_{ij} = \frac{\exp(w_{ij})}{\sum_{j=1}^D \exp(w_{ij})}$
...
Transform back to $\bm{v}_i$ as: $\bm{v}_i = \exp(\bm{w}_i \space \cdot \space {\bm{M}^{-1}})$
